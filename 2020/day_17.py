...
def initialise_grid(data, num_cycles):
    ...
    grid = np.zeros((grid_size['x'], grid_size['y'], grid_size['z'], grid_size['w']))
        ...
        for x in range(grid_shape['x']):
            for y in range(grid_shape['y']):
                for z in range(grid_shape['z']):
                    for w in range(grid_shape['w']):
                        num_active = 0
                        for delta in deltas:
                            x_delta, y_delta, z_delta, w_delta = x + delta[0], y + delta[1], z + delta[2], w + delta[3]
                            if 0 <= x_delta < grid_shape['x'] and \
                               0 <= y_delta < grid_shape['y'] and \
                               0 <= z_delta < grid_shape['z'] and \
                               0 <= w_delta < grid_shape['w']:
                                if grid_old[x_delta, y_delta, z_delta, w_delta]:
                                    num_active += 1
                        if grid_old[x, y, z, w]:
