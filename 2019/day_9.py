...
def command(mem):
    ...
    while True:
        ...
        # print('---------------------------------------------------------')
        # print('mem address 100 =', mem['memory'][100])
        ...
        # print('param_vals_inital', param_vals_initial)
        param_vals = []

        # change the parameters based on the mode (not used for opcode = 3 (input)
        if opcode == 3:
            param_vals.append(mem['memory'][param_vals_initial[0]])
        else:
        ...
        if opcode == 1:
            # print('address to write to = ', param_vals[2])
            mem['memory'][param_vals[2]] = param_vals[0] + param_vals[1]
        elif opcode == 2:
            mem['memory'][param_vals[2]] = param_vals[0] * param_vals[1]
        elif opcode == 3:
            mem['memory'][param_vals[0]] = int(input())
        elif opcode == 4:
            print(param_vals[0])
        elif opcode == 5:
            if param_vals[0] != 0:
            ...
            # print('rel base', mem['relative_base'])
        else:
...
example_init_mems = [
    ...
    {
        'memory': np.loadtxt('day_9.txt', delimiter=',', dtype=np.int32),
        'addr': 0,
        'relative_base': 0
    }
...
zero_array = np.zeros((5000,), dtype=np.int32)
